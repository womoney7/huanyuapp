...
namespace fastBinaryJSON
{
    ...
    public class BJSON
    {
        ...
        private TypeConverter _tc = new TypeConverter();
        ...
        {
            DynamicMethod dynamicGet = new DynamicMethod(string.Empty, typeof(object), new Type[] { typeof(object) }, type, true);
        ...
        internal object ParseDictionary(Dictionary<string, object> d, Dictionary<string, object> globaltypes, Type type)
        {
            object tn = "";
            ...
            foreach (string name in d.Keys)
            {
                ...
                if (pi.filled == true)
                {
                    ...
                    if (v != null)
                    {
                        ...
                        else if (pi.isClass && v is Dictionary<string, object> && pi.pt == typeof(System.Object))
                            oset = v;
                        ...
                        catch (InvalidCastException)
                        {
                            ...
                            if (oset is string)
                            {
                                if (pi.pt == typeof(int))
                                {
                                    n = int.Parse((string)oset);
                                }
                                else if (pi.pt == typeof(int?))
                                {
                                    n = int.Parse((string)oset);
                                }
                                else if (pi.pt == typeof(float))
                                {
                                    n = float.Parse((string)oset);
                                }
                                else if (pi.pt == typeof(float?))
                                {
                                    n = float.Parse((string)oset);
                                }
                                else if (pi.pt == typeof(double))
                                {
                                    n = double.Parse((string)oset);
                                }
                                else if (pi.pt == typeof(double?))
                                {
                                    n = double.Parse((string)oset);
                                }
                                else if (pi.pt == typeof(decimal))
                                {
                                    n = decimal.Parse((string)oset);
                                }
                                else if (pi.pt == typeof(decimal?))
                                {
                                    n = decimal.Parse((string)oset);
                                }
                                else if (pi.pt == typeof(DateTime))
                                {
                                    n = DateTime.Parse((string)oset);
                                }
                                else if (pi.pt == typeof(DateTime?))
                                {
                                    n = DateTime.Parse((string)oset);
                                }
                                else if (pi.pt == typeof(Enum))
                                {
                                    n = Enum.Parse(pi.pt, (string)oset);
                                }
                            }
                            else
                            {
                                n = _tc.ConvertTo(oset, pi.pt);
                            }
                            try
                            {
                                pi.setter(o, n);
                            ...
                            {
                                var errmsg = string.Format("数据类型转换错误，数据属性[{0}]-类型[{1}]转成类型[{2}]", pi.Name, oset.GetType().Name, pi.pt.Name);
